    ...
    function(oj, ko, $, app) {
        function FilmDetailViewModel() {
            ...
            self.handleActivated = function(info) { // 데이터를 읽어 오는 로직 구현
                console.log(JSON.stringify(app.router.getState('filmdetail').value));
                var url = app.router.getState('filmdetail').value['url'];
                ...
                $.get(url, function(json, status){
                    ...
                    self.starships([]);
                    for(var idx in json.starships) {
                        if(idx >= 5)
                            break;
                        
                        var starshipUrl = json.starships[idx];
                        $.get(starshipUrl, null, function(starship, status) {
                            self.starships.push({name: starship.name,
